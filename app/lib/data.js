...
export async function consultarRompimentos(q, page) {
  const regex = new RegExp(q, "i");
    const ITEM_PER_PAGE = 5;
  try {
      connectToDB();

      const rompimentos = await dbRompimento.find({ data_abertura: { $gte: new Date('2023-11-24') }})
      .limit(ITEM_PER_PAGE)
      .skip(ITEM_PER_PAGE * (page - 1));

      const count = await dbRompimento.find({ data_abertura: { $gte: new Date('2023-11-22') }}).count();
      
      return {rompimentos,count};
  } catch (err) {
      console.log(err);
